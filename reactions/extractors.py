...
class PostExtractor:
    ...
    total_reactions_regex = re.compile(r'>([0-9,.KM]+)<')
    top_reactions_regex = re.compile(r'u>(.+)</u')
    ...
    def extract_total(self) -> PartialPost:
        #total = self.element.find('footer div[data-sigil="reactions-sentence-container"] ._1g06', first=True).text
        return {
            'total_reactions': utils.find_and_search(
                self.element, 'footer div[data-sigil="reactions-sentence-container"] ._1g06', self.total_reactions_regex
            ) or 0,
        }

    def extract_top(self) -> PartialPost:
        tops = self.element.find('footer div[data-sigil="reactions-sentence-container"] u', first=False)
        return {
            'top_reactions': ",".join(top.text for top in tops),
        }
